...
@client.command(pass_context=True, aliases=['l', 'lea', 'fuckoff', 'die', 'disconnect'])
async def leave(ctx):
	channel = ctx.message.author.voice.channel
	voice = get(client.voice_clients, guild=ctx.guild)
...
async def resume(ctx):
	...
	if voice and voice.is_paused():
		print("Music resumed")
		voice.resume()
		await ctx.send("Your music is resumed")
	else:
...
async def stop(ctx):
	...
	if voice and voice.is_playing():
		print("Music stopped")
		voice.stop()
		await ctx.send("Stopped the music, do f!leave to make me leave")
	else:
